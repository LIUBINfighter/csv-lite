import { TextFileView, ButtonComponent, IconName } from "obsidian";
...
getIcon(): IconName {
  ...
  private setupColumnResize(handle: HTMLElement, columnIndex: number) {
    ...
    let tableHeaders: NodeListOf<HTMLElement>;
    let tableCells: NodeListOf<HTMLElement>;
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      // 获取所有相关列的单元格
      tableHeaders = this.tableEl.querySelectorAll('th');
      tableCells = this.tableEl.querySelectorAll(`tbody tr td:nth-child(${columnIndex + 1})`);

      // 添加正在调整的样式
      handle.addClass('resize-active');
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      // 直接更新DOM元素宽度，而不是重新渲染整个表格
      if (tableHeaders && tableHeaders[columnIndex]) {
        tableHeaders[columnIndex].style.width = `${width}px`;
      }
    ...
    const onMouseUp = () => {
      // 移除正在调整的样式
      handle.removeClass('resize-active');
  ...
  private adjustInputHeight(input: HTMLInputElement) {
    ...
    // 设置高度，确保能显示所有内容
    input.style.height = `${Math.max(minHeight, lineCount * lineHeight)}px`;
  }
